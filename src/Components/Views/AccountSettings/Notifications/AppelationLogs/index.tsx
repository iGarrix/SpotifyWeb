                ...
                reducer.appelations?.map(item => {
                    return (
                        <NotifyCard key={Guid.create().toString()} message={
                            <p className="w-full flex justify-between items-center"><span>Your message: {item.message}</span> <span className="font-medium text-white/80 whitespace-nowrap">{DeviceType.desktop}</span></p>
