...
export const NotifyCard: React.FC<INotifyCard> = ({ ...props }) => {
    return (
        <div className={`rounded-lg flex items-center gap-[20px] py-3 px-5 overflow-hidden ${props.isFunc && "transition-all cursor-pointer hover:shadow-xl"} ${props.type === "success" ? "bg-green-500/40" : props.type === "wait" ? "bg-primary-100/40" : "bg-red-500/40"}`} onClick={props.onClick}>
