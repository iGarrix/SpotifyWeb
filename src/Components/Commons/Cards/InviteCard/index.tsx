...
export const InviteCard: React.FC<IInviteCardProps> = ({...props}) => {
    ...
    return (
        <div className={`rounded-lg flex items-center gap-[20px] overflow-hidden bg-light-200 shadow-md py-2 px-5 select-none`}>
            ...
            <div className="flex flex-col justify-center">
                <div className="flex gap-2">
                    <p className="font-medium text-[19px] flex hover:text-primary-100 cursor-pointer" onClick={() => {nav("/album/" + props.invite.albomDto.returnId)}}>{props.invite.albomDto.name}</p>
            ...
            <div className="flex gap-2 z-[10]">
                <div className="bg-green-500 text-light-200 rounded-sm flex items-center justify-center text-center w-[35px] h-[35px]
                cursor-pointer transition-all hover:bg-primary-100" onClick={props.onAccept}>
                    <FontAwesomeIcon className="text-xl" icon={faCheck} />
                </div>
                <div className="bg-red-500 text-light-200 rounded-sm flex items-center justify-center text-center w-[35px] h-[35px]
