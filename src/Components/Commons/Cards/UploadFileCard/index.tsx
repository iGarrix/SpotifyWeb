    ...
    return (
        <div className="flex justify-between items-center rounded-lg gap-2 bg-light-200 p-2 pr-4">
            <div className="flex gap-3 items-center w-full">
                ...
                <input type={"text"} disabled={props.disable} className="whitespace-nowrap text-lg bg-transparent w-full border-0 outline-none" defaultValue={props.file.name} onChange={(e: any) => {props.onChange(e)}}/>
                {/* <p className="whitespace-nowrap text-lg">{props.file.title}</p> */}
            </div>
            <div className="flex items-center ml-auto gap-2">
                <p className="whitespace-nowrap">{formatBytes(props.file.file.size)}</p>
                <img className="invert w-[25px] cursor-pointer" alt="trash" src={icon_trash} onClick={props.onDelete} />
            </div>
        </div>
    )
}